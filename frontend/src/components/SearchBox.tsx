...
function SearchInput({ onFocusChange }: SearchInputProps) {
  ...
  // Dynamic width state container
  const [dropdownWidth, setDropdownWidth] = useState(65);

  // Amazon-Style Dynamic Width Calculator Matrix (Fine-tuned spacing)
  useEffect(() => {
    if (textMeasurementRef.current) {
      // Increased buffer padding to 44px to completely account for text size + custom arrow gap
      const calculatedWidth = textMeasurementRef.current.offsetWidth + 44;
      // Extended maximum range boundary to 240px to safely accommodate long text blocks
  ...
  // Close suggestions and overlay ONLY when clicking entirely outside the component workspace
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (suggestionRef.current && !suggestionRef.current.contains(e.target as Node)) {
        setShowSuggestions(false);
        if (onFocusChange) onFocusChange(false); // Triggers overlay close safely
      }
    };
  ...
  return (
    <div className="relative flex-1 max-w-sm" ref={suggestionRef}>
      ...
      {/* GHOST SPAN ENGINE: Renders text exactly to measure pure horizontal footprint */}
      <span
      ...
      >
        <select
          ...
          onFocus={() => {
            if (onFocusChange) onFocusChange(true); // Persist layout overlay on dropdown click
          }}
          onChange={(e) => {
            const nextCat = e.target.value;
            // Immediate route dispatch without dropping overlay context
            let targetUrl = `/search?`;
            const queryParts: string[] = [];
        ...
        <div className='w-full flex items-center bg-white'>
          <input
            ...
            onFocus={() => {
              ...
              if (onFocusChange) onFocusChange(true); // Keep overlay persistent
            }}
            onChange={(e) => {
